...
import { loginGetApi } from "../utils/apiFunctions";
import { texts, tr } from "../langs/tr";

export default function Login(): JSX.Element {
  document.title = tr(0, texts.login.loginPageTitle);
  ...
  const handleSubmit: (e: any) => void = (e: any) => {
    ...
    /[0-9]{10}/.test(userPhone) && userPassword
      ? (loginGetApi(userPhone, userPassword, navigate), setPhone(""), setPassword(""))
      : swal({
  ...
  return (
    <Form
      title={
        <LoginTexts
          title={tr(0, texts.login.loginFormTitle)}
          phoneText={tr(0, texts.formPlaceholders.FormFirstText)}
          text={tr(0, texts.login.loginFormSecondText)}
        />
      }
      onFormSubmit={(e: any) => handleSubmit(e)}
      switchButton={<Button title={tr(0, texts.login.loginFormButton)} link="/signup" />}
    />
  );
}
